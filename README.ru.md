...
История изменений — в [CHANGELOG.md](CHANGELOG.md). Ветка `main` может содержать изменения, которые новее последнего опубликованного релиза.
...
- **Автозапуск с Windows** для текущего пользователя.
...
- **Автозапуск** — включение/отключение запуска SleepMngr вместе с Windows для текущего пользователя.
...
Пункт **Автозапуск** записывает путь к текущему `SleepMngr.exe` в:
...
Используется `HKEY_CURRENT_USER`, поэтому включение и отключение автозапуска обычно не требует прав администратора. После изменения запись в реестре проверяется. Если затем перенести `SleepMngr.exe` в другую папку, выключите и снова включите **Автозапуск**, чтобы сохранить новый путь.

## Логирование

Логирование **выключено по умолчанию**, выбранное состояние сохраняется между запусками.

При включённом логе записываются смена режима, подключение/отключение дисплеев, операции с крышкой, настройка пробуждения от мыши, изменение автозапуска, команды и результаты `powercfg`, события сессии/питания, `Заснуть сейчас` и автоматические переходы в сон.
...
- Выключите и снова включите **Автозапуск**.
- Если EXE был перенесён, это обновит сохранённый путь.
- Если изменение реестра завершается ошибкой, включите лог и повторите действие.
...
AutoStartManager.cs         автозапуск текущего пользователя Windows
